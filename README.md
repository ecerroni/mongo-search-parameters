...
import { mongooseModels, UserDoc } from 'path-to-your-mongoose-models'
...
export default async () => {
  const categories = (await useSearchParams) < UserDoc > (User, { ...params })
  return categories
}
```
...
`age_ir: [21, 26]`: true if field is between 21 and 26 (exclusive)

### ire

`age_ire: [21, 26]`: true if field is between 21 and 26 (inclusive)
